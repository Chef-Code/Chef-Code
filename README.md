# 👨‍🍳 Chef-Code = (Lonnie Teter)³

<p align="center">
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Honk&size=30&duration=2500&pause=1000&color=2EF726&center=true&vCenter=true&width=500&lines=Tariff%2BAutomation%2BEngineer;Geospatial%2BSystems%2BBuilder;Owner%2Bof%2BWDA%2BMovers;Software%2BShould%2BRespect%2BOperations" alt="Typing SVG" /></a>
</p>

<p align="center">
  🚚 Owner of <strong>WDA Movers</strong> • 💻 Developer • 🧠 Systems Architect • 🗺️ Spatial Intelligence Builder
</p>

---

# ⚡ About Me

- 🏢 Owner of WDA Movers (Eugene, OR)
- 🧠 Building **WDA Systems** — Tariff-driven moving platform
- 🗺️ Developing **[OpenFence.AI](https://openfence.ai)** — Geofencing + dispatch intelligence
- 💻 Background in **C#**, backend systems, and operational software
- 📈 Focused on automation, logistics intelligence, and real-world tooling
- 🏀 Basketball coach with a competitor mindset
- 🎴 Hobby card collector. Still chasing that: **1/1 Superfractor Auto of [Linus Torvalds](https://cdn.mos.cms.futurecdn.net/Zn6XqZxCTKncLwX5TtrjEn-280-80.jpg.webp)** 
- ⚙️ Building software around actual operational constraints
- 🧑‍🍳 Oh Yeah Im also an **ACF Certified Chef** 🔪 **Mise En Place**

---

# 🛰️ Active Systems

| System | Status |
|---|---|
| 🚚 WDA Systems | 🟢 Operational |
| 🗺️ OpenFence | 🟢 Active Development |
| 📡 Dispatch Intelligence | 🟡 Training |
| ⚡ Tariff Automation Engine | 🟢 Online |
| 🤖 Business Automation Tools | 🟢 Deploying |

---

<table>
<tr>
<td align="center">

### 📊 GitHub Stats

<img src="https://github-readme-stats-fast.vercel.app/api?username=Chef-Code&show_icons=true&theme=tokyonight&hide_border=true"/>

</td>

<td align="center">

### 🔥 Current Streak

<img src="https://streak-stats.demolab.com?user=Chef-Code&theme=tokyonight&hide_border=true"/>

</td>
</tr>
</table>

---

# 🧠 Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=cs,python,fastapi,postgres,ts,js,docker,redis,git,vscode&theme=dark" />
</p>

---

# 🚀 Featured Projects

## 🚚 WDA Systems
Tariff-driven operational platform for household goods movers.

### Core Features
- ⚖️ Oregon tariff automation
- 🧾 Intrastate compliance engine
- 🗓️ Dispatch + scheduling intelligence
- 🗺️ Mapping & route systems
- 📦 Crew + truck optimization
- 🤖 AI-assisted operations

---

## 🗺️ OpenFence
Multi-tenant geospatial intelligence platform.

### Core Features
- 📡 Real-time geofencing
- 🛰️ Spatial event processing
- ⚡ Dispatch telemetry
- 🧠 Dwell + movement intelligence
- 🔐 Multi-tenant architecture
- 📈 Operational observability

---

# 📈 Profile Summary

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Chef-Code&theme=github_dark" />
</p>

---

# 📡 Current Focus

```diff
+ Building tariff intelligence systems
+ Spatial dispatch architecture
+ Geofencing event systems
+ Operational automation
+ Real-world workflow tooling
